...
export const ZATCA_LINKS = {
  ...
  opensslInstallWindows: {
    ...
    hint: "Download Win64 OpenSSL v3.x Light, enable Add to PATH during setup, then restart Nexttel POS",
  },
};
...
function getOpensslInstallHint() {
  if (detectPlatform() === "windows") {
    return "Install Win64 OpenSSL v3.x (Light edition is enough), enable Add to PATH during setup, then restart Nexttel POS.";
  }
  return "Install OpenSSL on your Mac, then restart Nexttel POS and try again.";
}
    ...
    return [
      ...
      },
      {
        ...
        help: [
          ...
          "This matches the Fatoora platform onboarding flow — generated automatically inside Nexttel POS.",
...
export function formatSetupError(error, stepId) {
  ...
  if (/XML-INVOICE-ERROR|not a Standard TAX document/i.test(raw)) {
    return {
      ...
      links: [ZATCA_LINKS.zatcaGuide],
      hint:
        "Clearance API is for Standard (B2B) tax invoices. Nexttel POS retail sales use Simplified invoices via the Reporting API. " +
